...
let bind_var_to_simple ~dbg env res v ~num_normal_occurrences_of_bound_vars s =
  ...
  | Some (alias_of, _coercion) when Flambda_features.classic_mode () ->
    Env.add_alias env res ~var:v
      ...
      ~alias_of
  | Some _ | None ->
    let defining_expr, free_names_of_defining_expr, env, res, effects_and_coeffects_of_defining_expr =
      C.simple ~dbg env res s
    in
    let env, res =
      Env.bind_variable env res v ~effects_and_coeffects_of_defining_expr
        ~defining_expr ~free_names_of_defining_expr ~num_normal_occurrences_of_bound_vars
    in
    env, res
...
let translate_apply0 env res apply =
  ...
  let callee, callee_free_names, env, res, _ = C.simple ~dbg env res callee_simple in
  ...
  | Call_kind.Method { kind; obj; alloc_mode } ->
    fail_if_probe apply;
    let obj, _obj_free_names, env, res, _ = C.simple ~dbg env res obj in
    let kind = Call_kind.Method_kind.to_lambda kind in
    let alloc_mode = Alloc_mode.For_types.to_lambda alloc_mode in
    C.send kind callee obj args (pos, alloc_mode) dbg, free_names, env, res, Ece.all
...
let translate_raise env res apply exn_handler args =
  ...
  | exn :: extra ->
    ...
    let extra, extra_free_names, env, res, _ = C.simple_list ~dbg env res extra in
...
let translate_jump_to_return_continuation env res apply return_cont args =
  ...
  | [return_value] -> (
    let dbg = Apply_cont.debuginfo apply in
    let return_value, free_names, env, res, _ = C.simple ~dbg env res return_value in
    let wrap, _, res = Env.flush_delayed_lets ~mode:Branching_point env res in
    match Apply_cont.trap_action apply with
    | None ->
      ...
      cmm, free_names, res
    | Some (Pop _) ->
      let cmm, free_names = wrap (C.trap_return return_value [Cmm.Pop]) free_names in
      cmm, free_names, res
    | Some (Push _) ->
      Misc.fatal_errorf
...
and let_cont_not_inlined env res k handler body =
  ...
  let vars, arity, handler, free_names_of_handler, res = continuation_handler env res handler in
  let catch_id, env =
  ...
  let cmm, free_names_of_body, res =
    ...
    then let_cont_exn_handler env res k body vars handler ~catch_id arity
      ...
      ( C.create_ccatch ~rec_flag:false ~body
        ...
        res )
  in
  let free_names = Backend_var.Set.union free_names_of_handler free_names_of_body in
  let cmm, free_names = wrap cmm free_names in
  cmm, free_names, res
  ...
  let conts_to_handlers, res =
    Continuation.Map.fold
      (fun k handler (conts_to_handlers, res) ->
        let vars, _arity, handler, free_names_of_handler, res = continuation_handler env res handler in
        Continuation.Map.add k (vars, handler, free_names_of_handler) conts_to_handlers, res)
      conts_to_handlers
  ...
  let handlers, free_names =
    Continuation.Map.fold
      (fun k (vars, handler, free_names_of_handler) (handlers, free_names) ->
         let free_names = Backend_var.Set.union free_names free_names_of_handler in
...
and continuation_handler env res handler =
  Continuation_handler.pattern_match' handler
    ~f:(fun params ~num_normal_occurrences_of_params:_ ~handler ->
      ...
      let free_names = List.fold_left (fun acc (cmm_var, _) ->
...
and switch env res switch =
  ...
  let untagged_scrutinee_cmm, scrutinee_free_names, env, res, _ = C.simple ~dbg env res scrutinee in
  ...
  | 2 -> (
    ...
    | (0, else_, else_free_names, else_dbg), (_, then_, then_free_names, then_dbg)
    | (_, then_, then_free_names, then_dbg), (0, else_, else_free_names, else_dbg) ->
      let free_names =
        Backend_var.Set.union scrutinee_free_names
          (Backend_var.Set.union else_free_names then_free_names)
      in
      let cmm, free_names = wrap (C.ite ~dbg scrutinee ~then_dbg ~then_ ~else_dbg ~else_) free_names in
      cmm, free_names, res
    ...
    | (x, if_x, if_x_free_names, if_x_dbg), (_, if_not, if_not_free_names, if_not_dbg) ->
      let free_names =
        Backend_var.Set.union scrutinee_free_names
    ...
    let _, res, free_names =
      Targetint_31_63.Map.fold
        (fun discriminant action (i, res, free_names) ->
          ...
          index.(d) <- i;
          i + 1, res, free_names)
        arms (0, res, scrutinee_free_names)
    in
