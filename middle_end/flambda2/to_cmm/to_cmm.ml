...
let unit0 ~offsets flambda_unit ~all_code =
  ...
  let body, body_free_names, res = To_cmm_expr.expr env r (Flambda_unit.body flambda_unit) in
  let free_names = Backend_var.Set.remove (Backend_var.With_provenance.var toplevel_region_var) body_free_names in
    if not (Backend_var.Set.is_empty free_names) then
      Misc.fatal_errorf "Unbound free_names in module init code when translating to cmm: %a"
