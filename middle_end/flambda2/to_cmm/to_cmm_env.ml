...
type _ bound_expr =
  | Simple : { cmm_expr : Cmm.expression; free_names: free_names; } -> simple bound_expr
  | Split : { cmm_expr : Cmm.expression; free_names: free_names; } -> complex bound_expr
  | Splittable_prim :
      { dbg : Debuginfo.t;
...
let create_binding (type a) effs var ~(inline : a inline)
  ...
  | (Split { cmm_expr; free_names; }
    | Simple { cmm_expr; free_names; }) when is_cmm_simple cmm_expr ->
  ...
  let (new_bindings, _, free_names_of_new_cmm_args), new_cmm_args =
    List.fold_left_map
      (fun (new_bindings, order, free_names) (cmm_arg, arg_effs, arg_free_names) ->
        ...
        then (new_bindings, order, Backend_var.Set.union free_names arg_free_names), cmm_arg
        else
          ...
          let backend_var = Backend_var.create_local (Format.asprintf "to_cmm_split_%d" order) in
          let new_cmm_var = Backend_var.With_provenance.create ?provenance:None backend_var in
          let binding =
            Binding
              { order;
                effs = arg_effs;
                inline = Do_not_inline;
                bound_expr = Simple { cmm_expr = cmm_arg; free_names = arg_free_names; };
                cmm_var = new_cmm_var
              }
          in
          ( (binding :: new_bindings, order - 1, Backend_var.Set.add backend_var free_names),
            C.var backend_var))
  ...
  | Splittable_prim { dbg; prim; args } ->
    ...
    let split_binding =
      { order = binding.order;
        ...
        bound_expr = Split { cmm_expr = new_cmm_expr; free_names = free_names_of_new_cmm_args; };
...
let bind_variable ?extra env res var ~defining_expr
  ...
  | Must_inline_once ->
    let defining_expr = complex_no_split defining_expr free_names_of_defining_expr in
    bind_variable_with_decision ?extra env res var
      ~effects_and_coeffects_of_defining_expr ~defining_expr
...
let will_inline_simple env res { effs; bound_expr = Simple { cmm_expr; free_names; }; _ } =
...
let will_inline_complex env res { effs; bound_expr; _ } =
  ...
  | Splittable_prim { dbg; prim; args } ->
    let free_names, cmm_args =
      List.fold_left_map (fun free_names (cmm_arg, _ece, arg_free_names) ->
...
let add_alias env res ~var ~alias_of ~num_normal_occurrences_of_bound_vars =
  ...
  | Binding { inline = (Must_inline_once | Must_inline_and_duplicate); _ }, _ ->
    ...
    let cmm_expr, free_names, env, res, ece = inline_variable env res alias_of in
  ...
  let wrap_flush order_map e free_names =
    M.fold
      (fun _ (Binding b) (acc, acc_free_names) ->
        ...
        | Split { cmm_expr; free_names; }
        | Simple { cmm_expr; free_names; } ->
          let expr = Cmm_helpers.letin b.cmm_var ~defining_expr:cmm_expr ~body:acc in
