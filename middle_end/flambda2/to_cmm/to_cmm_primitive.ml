...
let arg_list ?consider_inlining_effectful_expressions ~dbg env res l =
  ...
  let args, free_names, env, res, effs =
    List.fold_left aux ([], Backend_var.Set.empty, env, res, Ece.pure_can_be_duplicated) l
  in
...
let prim_simple env res dbg p =
  ...
  (* Somewhat counter-intuitively, the left-to-right translation below (e.g. [x]
     ...
     order---ensuring maximal inlining---since [arg_list] translates the
  ...
  | Ternary (ternary, x, y, z) ->
    ...
    let free_names = Backend_var.Set.union (Backend_var.Set.union x_free_names y_free_names) z_free_names in
...
let prim_complex env res dbg p =
  ...
  let prim', args, effs, env, res =
    ...
    | Ternary (ternary, x, y, z) ->
      ...
      let z, z_free_names, env, res, effz = arg env res z in
      let effs = Ece.join (Ece.join effx effy) effz in
      prim', [x, effx, x_free_names; y, effy, y_free_names; z, effz, z_free_names], effs, env, res
    | Variadic (((Make_block _ | Make_array _) as variadic), l) ->
