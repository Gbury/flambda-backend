...
let name0 ?consider_inlining_effectful_expressions env res name =
  Name.pattern_match name
    ...
    ~symbol:(fun s ->
      ...
      symbol ~dbg:Debuginfo.none s, Backend_var.Set.empty, env, res, Ece.pure_can_be_duplicated)
...
let simple ?consider_inlining_effectful_expressions ~dbg env res s =
  Simple.pattern_match s
    ...
    ~const:(fun c -> const ~dbg c, Backend_var.Set.empty, env, res, Ece.pure_can_be_duplicated)
...
let make_update env res dbg kind ~symbol var ~index ~prev_updates =
  ...
  | Some prev_updates -> env, res, free_names, Some (sequence prev_updates update)
