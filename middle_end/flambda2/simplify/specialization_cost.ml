...
let add_prim bound_var prim t =
  update_cost t ~f:(fun { primitives } ->
      (* CR gbury: we might want to limit the number of primitives we store,
         so that e.g. if there are more than <n> primitives, we stop storing
         them and decide that no specialization should take place *)
...
let cost_metrics typing_env ~switch ~join_info ~specialized ~generic (cost : cost) =
  ...
  let metrics =
    ...
    | [] ->
        (* CR gbury: in this case, we actually remove the code size of the handler, since
           it disappears in favor of the specialized calls; we'll take into account the size of each
           specialized handler later, so here we need to count the negative of the code size of the generic
           handler *)
        let code_size =
          List.fold_left (fun acc (_, primitive) ->
              Code_size.(+) acc (Code_size.prim primitive)
            ) (Code_size.switch switch) cost.primitives
        in
        Cost_metrics.from_size (Code_size.(-) Code_size.zero code_size)
    | _ :: _ ->
        (* The regular case, where the generic continuaion handler stays, and the code size is
           not changed (at least for the generic case *)
        Cost_metrics.zero
  in
  ...
  in
  (* For each primitive, and each specialized call site:
     - notify it removed if its value is known
     - else add the prim to the code size *)
  List.fold_left (fun metrics (bound_var, prim) ->
      let simple = Simple.var (Bound_var.var bound_var) in
      let canonical_simple =
        Typing_env.get_canonical_simple_exn typing_env ~min_name_mode:Name_mode.in_types simple
      in
      let disappears, stays =
        Simple.pattern_match canonical_simple
          ~name:(fun name ~coercion:_ ->
              ...
              | Known { known_at_uses; unknown_at_uses } -> known_at_uses, unknown_at_uses )
          ~const:(fun _ ->
              (* if the canonical is a simple already, then specialization does not change much *)
              [], []
            )
      in
      let metrics =
        List.fold_left (fun acc _ ->
            Cost_metrics.notify_removed ~operation:(Removed_operations.prim prim) acc
          ) metrics disappears
      in
      let metrics =
        List.fold_left (fun acc _ ->
